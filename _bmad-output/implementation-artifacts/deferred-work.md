...
- source_spec: none
  summary: `.github/workflows/claude.yml` (installed via `/install-github-app`) fails on any PR that adds new files when triggered by an `@claude` mention in a comment (`issue_comment`/`pull_request_review_comment`) — a known, filed upstream limitation, not specific to this repo.
  evidence: Reproduced directly on PR #4: the action logs "PR head is untrusted", restores/checks out `origin/main` instead of the PR's actual head, then fails with `fatal: could not open '<new file>' for reading: No such file or directory` while trying to `git hash-object` every file the PR newly added, for context-building. Confirmed via `anthropics/claude-code-action` issue #962 and its own `docs/security.md` — `issue_comment`/`pull_request_review_comment` events don't carry full PR-head context the way `pull_request` events do, so the action conservatively treats the PR head as untrusted. Affects essentially every feature PR in this project (they all add new files). The documented fix (switch to `pull_request: [opened, synchronize, reopened]` triggers) was considered and declined — it would make `claude.yml` functionally redundant with the already-working `claude-code-review.yml`, which auto-reviews every PR via a proper `pull_request` trigger and doesn't hit this bug. Decision: leave `claude.yml` as installed; `@claude` mentions will keep failing on PRs with new files until upstream resolves #962. Revisit if that issue closes, or if `@claude`-mention replies become more important than they are today.
